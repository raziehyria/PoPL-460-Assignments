#|
Group 1: Mathyo Abou Asali, Razie Hyria
...
;; interp ----------------------------------------
(define (interp-helper vl nl body)
  (if (not (= (length vl) (length nl)))
      (error 'interp "wrong arity")
  (cond
    [(empty? nl) body]
    [else (interp-helper (rest vl) (rest nl) (subst  (first vl) (first nl) body))])))
(define (interp [a : Exp] [defs : (Listof Func-Defn)]) : Number
  (type-case Exp a
    ...
    [(appE s args) (local [(define fd (get-fundef s defs))]
                    (interp (interp-helper (map (lambda (arg) (numE (interp arg defs))) args)
                                   (fd-args fd)
                                   (fd-body fd))
                            defs))]))
...
(module+ test
  ...
  (test (subst (parse `8) 'x (parse `{double x}))
        (parse `{double 8}))
  (test (subst (parse `8) 'x (parse `{max x y}))
        (parse `{max 8 y}))
  )
(test (interp (parse `{max 1 2})
              (list))
