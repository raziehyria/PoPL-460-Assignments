...
(define-type Exp
  ...
  (firstE [a : Exp])
  (restE [a : Exp])
  (if0E [tst : Exp] ;;Part 1 — Inferring Conditional Types
        [thn : Exp]
        [els : Exp]))
  ...
  (type-case Exp a
    ...
    [(firstE a) (type-case Value (interp a env)
                  [(listV elems) (if (empty? elems)
                                     ...
                                     (first elems))]
                  [else (error 'interp "not a list")])]
    [(if0E tst thn els) (type-case Value (interp tst env)
     [(numV n) (if (= n 0) (interp thn env) (interp els env))]
     ...
     (local [(define result-type (varT (box (none))))]
       (begin
         (unify! (arrowT (typecheck arg tenv)
                 ...
                 fun)
         result-type))]
    [(if0E tst thn els) (begin
                          (unify! (typecheck tst tenv) (numT) tst)
                          (let ([thn-type (typecheck thn tenv)])
                            (begin
                              (unify! thn-type (typecheck els tenv) els)
                              thn-type)))]
    [(emptyE) (listofT (varT (box (none))))]
    [(consE l r)
     (let ([l-type (listofT (typecheck l tenv))])
       (begin
         (unify! l-type (typecheck r tenv) r)
         l-type))]
    [(firstE a) (let [(fst-type (varT (box (none))))]
                  (begin
                    (unify! (listofT fst-type) (typecheck a tenv) a)
                    fst-type))]
    [(restE a)  (let [(rst-type (listofT (varT (box (none)))))]
                  (begin
                    (unify! rst-type (typecheck a tenv) a)
                    rst-type))]))
...
(define (run-prog [s : S-Exp]) : S-Exp
  (let ([e (parse s)])
    (begin
      (typecheck e mt-env)
      (type-case Value (interp e empty)
...
(module+ test
  ...
  (test (parse-type `(Listof num))
        (listofT (numT)))
  (test/exn (interp (parse `{if0 {lambda {[x : bool]} x} 6 9}) mt-env)
            "not a number")
